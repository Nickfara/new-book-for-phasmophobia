...
function getLink(){
    try{
        ...
        if(d){
            ...
            fetch(`http://localhost:8000/zn/difficulties/${discord_user['id']}`, {signal: AbortSignal.timeout(6000)})
            .then(data => data.json())
            .then(data => {
                data.forEach(dif => {
                    var opt = document.createElement('option');
...
function save_difficulty(){
    ...
    .then(data => {
        ...
        fetch(`http://localhost:8000/zn/difficulties/${discord_id}/${url_id}?name=${encodeURIComponent(name)}`, {method:"POST",signal: AbortSignal.timeout(6000)})
        .then(data => {
            let presets = document.getElementById("saved-ids")
            var opt = document.createElement('option');
            opt.value = url_id;
