...
function checkLink(){
    return new Promise((resolve, reject) => {
        ...
        if (params.get("id")){
            ...
            fetch(`http://localhost:8000/zn/${znid}/attach/${discord_link['id']}`, {method:"POST",signal: AbortSignal.timeout(6000)})
            .then(data => {
...
function heartbeat(){
    if(znid != "no-connection-to-server"){
        ...
        fetch("http://localhost:8000/zn/"+znid,{method:"POST",Accept:"application/json",body:JSON.stringify(state),signal: AbortSignal.timeout(10000)})
...
function loadAllAndConnect(){
    let loadZN = new Promise((resolve, reject) => {
        ...
        if(znid && znid!="no-connection-to-server"){
            ...
            fetch(`http://localhost:8000/zn/?lang=${lang}${id ? '&discord_id='+id : ''}`,{headers:{Accept:"application/json"}, signal: AbortSignal.timeout(10000)})
            .then(e=>e.json())
            .then(e => {
                znid = e.znid
    ...
    let loadData = new Promise((resolve, reject) => {
        ...
        try{
            fetch(`http://localhost:8000/phasmophobia/data/ghosts.json?lang=${lang}`, {cache: 'default', signal: AbortSignal.timeout(10000)})
            .then(data => data.json())
            .then(data => {
    ...
    let loadMaps = new Promise((resolve, reject) => {
        fetch("http://localhost:8000/phasmophobia/data/maps", {cache: 'default', signal: AbortSignal.timeout(12000)})
        .then(data => data.json())
        .then(data => {
            var map_html = ""
            var usr_set = {}
    ...
    let loadWeekly = new Promise((resolve, reject) => {
        fetch("http://localhost:8000/phasmophobia/data/weekly.json", {cache: 'default', signal: AbortSignal.timeout(10000)})
        .then(data => data.json())
        .then(data => {
            weekly_data = {
            ...
            document.getElementById("weekly_footer").innerHTML = `<b>{{weekly_difficulty_settings}}</b>: <a href="http://localhost:8000/phasmo-cheat-sheet/difficulty-builder/?share=${weekly_data.difficulty_id}" target="_blank">${weekly_data.difficulty_id}</a>`
    ...
    let loadLanguages = new Promise((resolve, reject) => {
        fetch("http://localhost:8000/phasmophobia/languages", {cache: 'default', signal: AbortSignal.timeout(10000)})
        .then(data => data.json())
        .then(data => {
            var lang_html = ""
