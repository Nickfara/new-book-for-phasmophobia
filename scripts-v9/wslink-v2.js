...
function create_room(){
    ...
    fetch(`https://phasmophobia-room-server.onrender.com/znlink/create-room/${znid}`,{method:"POST",Accept:"application/json",body:JSON.stringify(outgoing_state),signal: AbortSignal.timeout(6000)})
...
function create_link(auto_link = false){
    ...
    relink_timeout = null
    fetch(`https://phasmophobia-room-server.onrender.com/znlink/create-link/${znid}`,{method:"POST",Accept:"application/json",signal: AbortSignal.timeout(6000)})
    .then(response => response.json())
    .then(data => {
...
function link_room(){
    ...
    ws = new WebSocket(`wss://phasmophobia-room-server.onrender.com/phasmolink/link/${znid}/${room_id}${load_pos ? '?pos='+load_pos : ''}`);
    ...
    ws.onerror = function(event){
        ...
        setCookie("room_id","",-1)
        document.getElementById("map-explorer-link-2").href = `https://nickfara.github.io/new-book-for-phasmophobia/phasmo-cheat-sheet/map-explorer/`
    }
    ws.onmessage = function(event) {
        try {
            ...
            if (incoming_state.hasOwnProperty("setpos")){
                ...
                document.getElementById("map-explorer-link-2").href = `https://nickfara.github.io/new-book-for-phasmophobia/phasmo-cheat-sheet/map-explorer/?jlid=${room_id}&pos=${my_pos}&share=${lmap}`
                if($(".guessed").length > 0){
...
function link_link(reconnect = false){
    ...
    try{
        dlws = new WebSocket(`wss://phasmophobia-room-server.onrender.com/phasmolink/link/${link_id}${reconnect ? '?reconnect=true' : ''}`);
    }
    catch(e){
        relink_live = false
        return
...
function disconnect_room(reset=false,has_status=false){
    ...
    var lmap = document.getElementsByClassName("selected_map")[0].id
    document.getElementById("map-explorer-link-2").href = `https://nickfara.github.io/new-book-for-phasmophobia/phasmo-cheat-sheet/map-explorer/?share=${lmap}`
    if (Object.keys(discord_user).length == 0)
